...
public class MecanumDrive{
    ...
    public void init(HardwareMap HM) {
        ...
        InL = HM.dcMotor.get("InL"); InR = HM.dcMotor.get("InR"); // map the intake
        releaseIn = HM.servo.get("releaseIn"); // map release
        PlateGrabL = HM.servo.get("PlateGrabL"); PlateGrabR = HM.servo.get("PlateGrabR"); // map plate grabbers
    ...
    public void zeroALL() { // Zeros all the functions
        flWheelPower = blWheelPower = frWheelPower = brWheelPower = 0;
        releaseInPos = PlateGrabLPos = PlateGrabRPos = InRPower = InLPower = 0;
    }
    ...
    double flWheelPower, blWheelPower, frWheelPower, brWheelPower; // Power of Wheels
    double releaseInPos, PlateGrabLPos, PlateGrabRPos, InLPower, InRPower; // release, plateGrabbers, and intake powers
     ...
     */
    public void DriveTrain(double left_stick_x,double left_stick_y, double right_stick_x) {// the math for the mecanum wheel
        flWheelPower = -left_stick_y + right_stick_x + left_stick_x;
        blWheelPower = -left_stick_y + right_stick_x - left_stick_x;
        frWheelPower = -left_stick_y - right_stick_x - left_stick_x;
        brWheelPower = -left_stick_y - right_stick_x + left_stick_x;
    }

    public void DriveTrain(double x) { //Strafing
        flWheelPower = frWheelPower = x;
        blWheelPower = brWheelPower = -x;
    }

    public void SetMotorPower(){ // Sets the power for ALL motors and servos
        fl.setPower(flWheelPower); bl.setPower(blWheelPower);
        fr.setPower(frWheelPower); br.setPower(brWheelPower);
